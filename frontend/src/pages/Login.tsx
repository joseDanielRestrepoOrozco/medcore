...
const Login = () => {
  ...
  const onSubmit = form.handleSubmit(async (data) => {
    ...
    setError(null);
    try {
      const res = await login({ email: data.email, currentPassword: data.currentPassword });
      ...
      const ax = err as { response?: { status?: number; data?: { error?: unknown } } };
      ...
      if (status === 401) {
        if (msg.includes('email no verificado')) {
          ...
          form.setError('email', { type: 'server', message: 'Correo pendiente por verificación' });
        } else {
          setError('Correo o contraseña incorrectos.');
          form.setError('email', { type: 'server', message: 'Revisa tu correo' });
          form.setError('currentPassword', { type: 'server', message: 'Revisa tu contraseña' });
        }
      } else if (status === 400) {
    ...
    <div className="flex items-center justify-center min-h-[calc(100vh-11rem)] bg-gray-50">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-10">
        ...
        <p className="mt-6 text-center text-slate-600">
          ¿No tienes cuenta?{' '}
          <a href="/solicitar-registro" className="text-slate-800 font-semibold hover:underline">
            Solicitar registro
          </a>
        </p>
      </div>
